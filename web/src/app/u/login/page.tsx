...
import { setStoredUserToken } from "@/store/user-auth";
...
export default function UserLoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      await setStoredUserToken(normalized);
      router.replace("/u/image");
    } catch (error) {
        ...
        <CardContent className="space-y-7 p-6 sm:p-8">
          <div className="space-y-4 text-center">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl font-semibold tracking-tight text-stone-950">AI 画图工作台</h1>
              <p className="text-sm leading-6 text-stone-500">输入管理员分发的访问令牌继续使用。</p>
            </div>
          </div>

          <div className="space-y-3">
            <label htmlFor="user-token" className="block text-sm font-medium text-stone-700">
              访问令牌
            </label>
            <Input
              id="user-token"
