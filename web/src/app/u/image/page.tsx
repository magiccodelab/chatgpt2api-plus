...
import { fetchUserMe, userEditImage, userGenerateImage, type UserQuotaStatus } from "@/lib/user-api";
import { clearStoredUserToken } from "@/store/user-auth";
import {
...
const CONVERSATION_NAMESPACE = "user";
const ACTIVE_CONVERSATION_STORAGE_KEY = "chatgpt2api:user_image_active_conversation_id";
...
async function recoverConversationHistory(items: ImageConversation[]) {
  ...
  if (changedConversations.length > 0) {
    await saveImageConversations(normalized, CONVERSATION_NAMESPACE);
  }
...
export default function UserImagePage() {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        const items = await listImageConversations(CONVERSATION_NAMESPACE);
  ...
  const loadQuota = useCallback(async () => {
    try {
      ...
      // 401 已由 interceptor 处理重定向；其他错误保留上次状态
    }
  }, []);
  ...
  const persistConversation = async (conversation: ImageConversation) => {
    ...
    await saveImageConversations(nextConversations, CONVERSATION_NAMESPACE);
  };
  ...
  const handleDeleteConversation = async (id: string) => {
    ...
    try {
      await deleteImageConversation(id, CONVERSATION_NAMESPACE);
    } catch (error) {
  ...
  const handleClearHistory = async () => {
    try {
      await clearImageConversations(CONVERSATION_NAMESPACE);
  ...
  const runConversationQueue = useCallback(
    async (conversationId: string) => {
      if (activeConversationQueueIds.has(conversationId)) return;

      const snapshot = conversationsRef.current.find((conversation) => conversation.id === conversationId);
      const queuedTurn = snapshot?.turns.find((turn) => turn.status === "queued");
      if (!snapshot || !queuedTurn) return;
        ...
        return {
          ...
          turns: conversation.turns.map((turn) =>
            turn.id === queuedTurn.id
              ? { ...turn, status: "generating", error: undefined }
              : turn,
          ),
        };
      });
          ...
          try {
            const data =
              queuedTurn.mode === "edit"
                ? await userEditImage(referenceFiles, queuedTurn.prompt)
                : await userGenerateImage(queuedTurn.prompt);
            const first = data.data?.[0];
            if (!first?.b64_json) throw new Error("未返回图片数据");
            if (data.usage) setQuotaStatus(data.usage);
  ...
  const handleSubmit = async () => {
    ...
    const draftTurn: ImageTurn = {
      ...
      count: 1,
      images: [{ id: `${turnId}-0`, status: "loading" as const }],
      createdAt: now,
      status: "queued",
    };
  ...
  const quotaSlot = useMemo(() => {
    if (!quotaStatus) {
      return (
        <div className="rounded-full bg-stone-100 px-3 py-2 text-xs font-medium text-stone-600">加载额度中...</div>
      );
    }
    const { remaining, daily_limit, reset_at } = quotaStatus;
    const resetLabel = formatResetTime(reset_at);
    return (
      <div className="flex items-center gap-2 rounded-full bg-stone-100 px-3 py-2 text-xs font-medium text-stone-600">
        <span>今日剩余 {remaining} / {daily_limit}</span>
        {resetLabel ? <span className="text-stone-400">· 下次重置 {resetLabel}</span> : null}
      </div>
    );
  }, [quotaStatus]);
  ...
  return (
    <>
      <header className="flex h-12 items-center justify-between pt-1">
        <div className="flex items-center gap-3">
          <span className="py-2 text-[15px] font-semibold tracking-tight text-stone-950">AI 画图工作台</span>
          {quotaStatus?.name ? (
      ...
      <section className="mx-auto grid h-[calc(100vh-5rem)] min-h-0 w-full max-w-[1380px] grid-cols-1 gap-3 px-3 pb-6 lg:grid-cols-[240px_minmax(0,1fr)]">
        ...
        <div className="flex min-h-0 flex-col gap-4">
          ...
          <ImageComposer
            ...
            prompt={imagePrompt}
            imageCount="1"
            availableQuota={quotaStatus ? String(quotaStatus.remaining) : "--"}
